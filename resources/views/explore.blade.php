...
<section x-data="FeedPage()" x-init="init()" class="grid grid-cols-1 md:grid-cols-[260px,minmax(0,1fr),300px] gap-4 lg:gap-6">
  ...
  <div>
    <!-- Composer -->
    <div class="glass rounded-2xl p-3 sm:p-4 mb-3 flex items-center gap-3">
      <img src="/images/demo/rina.jpg" class="w-9 h-9 rounded-full object-cover" alt="">
      <input class="flex-1 rounded-xl px-3 py-2 border" placeholder="Bagikan karya atau gambar..." @focus="location.href='/submit'">
      <a href="/submit" class="px-3 py-2 rounded-xl btn-accent text-white">Unggah</a>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 mb-3">
      ...
      <div class="ml-auto flex items-center gap-2">
        ...
        <!-- Toggle Semua/Diikuti (pakai style .toggle dari layout) -->
        <button
          @click="mode = (mode==='all' ? 'following' : 'all')"
          :class="['toggle', mode==='following' ? 'is-active' : '']">
      ...
      <template x-for="p in paged" :key="p.id">
        <article class="glass rounded-2xl overflow-hidden hover:shadow transition">
          ...
          <div class="p-3">
            ...
            <!-- Aksi -->
<!-- Aksi (ikon-only footer) -->
<div class="icon-row">
  <!-- Like -->
  <button class="icon-btn"
          :class="liked.has(p.id)?'is-active like':''"
          :title="likeCount(p)+' suka'"
          @click="toggleLike(p.id)">
    <!-- heart -->
    <svg viewBox="0 0 24 24" class="icon">
      <path d="M12 21s-7-4.35-9.33-8.12A5.5 5.5 0 0 1 12 6.09a5.5 5.5 0 0 1 9.33 6.79C19 16.65 12 21 12 21z"/>
    </svg>
  ...
  <!-- Save -->
  <button class="icon-btn"
          :class="saved.has(p.id)?'is-active save':''"
          :title="saved.has(p.id)?'Tersimpan':'Simpan'"
          @click="toggleSave(p.id)">
    <!-- pin -->
    <svg viewBox="0 0 24 24" class="icon">
      <path d="M14 2l-1 7 3 3-4 1-6 7 1-6-4-4 7-1z"/>
    </svg>
  </button>
  ...
  <button class="icon-btn" title="Bagikan" @click="share(p)">
    <!-- share -->
    <svg viewBox="0 0 24 24" class="icon">
      <path d="M14 9l-4 2 4 2m5-9a3 3 0 1 1-6 0 3 3 0 0 1 6 0zM5 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm13 9a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"/>
    </svg>
  </button>
</div>
...
<!-- Date -->
<div class="card-date">
  <span class="date-badge" x-text="formatDate(p.date)"></span>
</div>
  ...
  <aside class="hidden lg:block glass rounded-2xl p-3 h-fit sticky top-24 space-y-4">
    ...
    <div>
      ...
      <template x-for="u in whoToFollow" :key="u.username">
        <div class="flex items-center gap-2 p-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/5">
...
<script>
  function FeedPage(){
    ...
    // helper tanggal: 14/07/25 • 3 hari lalu
    const timeAgo = (d)=>{
  ...
  // badge tanggal: "14/07/25 · 3 hari lalu"
  ...
  async init(){
    ...
    window.addEventListener('storage', (e)=>{
      ...
      if(e.key==='hs-follow-users'){ this.followUsers = new Set(LS.get('hs-follow-users',[])); this.refreshWhoToFollow(); }
  ...
  get filtered(){
    ...
    return this.all.filter(p => {
      ...
      const bySelectedTag = this.selectedTags.size === 0
        || p.tags.some(t => this.selectedTags.has(t));

      const byFollowedTag = this.followedTags.size === 0
        || p.tags.some(t => this.followedTags.has(t));

      const byFollowUser = !onlyFollowing || this.followUsers.has(p.author);

      return byText && bySelectedTag && byFollowedTag && byFollowUser;
    });
  },
  ...
  get trendingTags(){
    const m = new Map();
    this.filtered.forEach(p => p.tags.forEach(t => m.set(t, (m.get(t)||0)+1)));
  ...
  get whoToFollow(){ return this._whoToFollow || []; },
  ...
  toggleFollowUser(u){ this.followUsers.has(u)?this.followUsers.delete(u):this.followUsers.add(u); LS.set('hs-follow-users',[...this.followUsers]); this.refreshWhoToFollow(); },
  isFollowingUser(u){ return this.followUsers.has(u); },
  refreshWhoToFollow(){ const pool=this.users.filter(u=>!this.followUsers.has(u.username)).slice(0,5); this._whoToFollow=pool; }
}
